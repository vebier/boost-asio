...
void CSession::Start(){
	...
	boost::asio::async_read(_socket, boost::asio::buffer(_recv_head_node->_data, HEAD_TOTAL_LEN),
		bind(&CSession::HandleHeadRead, this, placeholders::_1, placeholders::_2, SharedSelf()));*/
}
...
void CSession::HandleHeadRead(const boost::system::error_code& error, size_t bytes_transferred, std::shared_ptr<CSession> shared_self)
{
	try {
		if (!error)
		{
			...
			{
				cout << "read head lenth error" << endl;
				Close();
			...
			short msg_len = 0;
			memcpy(&msg_len, _recv_head_node->_data + 2, HEAD_DATA_LEN);
			...
			boost::asio::async_read(_socket, boost::asio::buffer(_recv_msg_node->_data, _recv_msg_node->_total_len),
				bind(&CSession::HandleMsgRead, this, placeholders::_1, SharedSelf()));
...
void CSession::HandleMsgRead(const boost::system::error_code& error, std::shared_ptr<CSession> shared_self)
{
	try
	{
		if (!error) 
		{
			...
			_recv_head_node->Clear();
			boost::asio::async_read(_socket, boost::asio::buffer(_recv_head_node->_data, _recv_head_node->_total_len),
				bind(&CSession::HandleHeadRead, this, placeholders::_1, placeholders::_2, SharedSelf()));
		}
		else
		{
